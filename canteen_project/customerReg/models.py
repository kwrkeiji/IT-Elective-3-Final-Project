...
class CustomerReg(models.Model):
    ...
    image = models.ImageField(upload_to="customer_images/", blank=True, null=True, validators=[validate_image])
...
@receiver(post_delete, sender=CustomerReg)
...
@receiver(pre_save, sender=CustomerReg)
def delete_old_image_on_profile_update(sender, instance, **kwargs):
    if not instance.pk:
        return  # Skip if the instance is new

    try:
        old_instance = CustomerReg.objects.get(pk=instance.pk)
    except CustomerReg.DoesNotExist:
        return
